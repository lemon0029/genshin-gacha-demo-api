...
import me.xuqu.config.AppProperties;
import me.xuqu.model.entity.item.GenshinCharacter;
import me.xuqu.model.entity.item.GenshinItemType;
...
public class FetchGenshinItemData implements InitializingBean {
            ...
            if (jsonBody.optInt("retcode", -1) != 0 || !"OK".equalsIgnoreCase(jsonBody.optString("message"))) {
                ...
                for (int i = 0; i < dataList.length(); i++) {
                    ...
                    if (genshinCharacterOptional.isEmpty()) {
                        ...
                        int elementAttrId = switch (element) {
                            case "Pyro" -> 1;
                            case "Anemo" -> 2;
                            case "Geo" -> 3;
                            case "Electro" -> 5;
    ...
    private List<Header> initHeaderOfCookie() {
        ...
        String format = "account_id=%s;cookie_token=%s".formatted(accountId, cookieToken);
